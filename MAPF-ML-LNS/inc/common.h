...
struct IterationStats
{
    ...
    int model;
    IterationStats(int num_of_agents, int sum_of_costs, double runtime, string algorithm, int model,
            int sum_of_costs_lowerbound = 0) :
            num_of_agents(num_of_agents), sum_of_costs(sum_of_costs), runtime(runtime),
            sum_of_costs_lowerbound(sum_of_costs_lowerbound), algorithm(algorithm),model(model) {}
};
