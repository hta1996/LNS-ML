...
std::pair<std::vector<Constraint>, std::vector<Constraint>> ConstraintPropagation::generate_constraints(int level_0, int level_1){
  ...
  for (auto& it:cons_0){
    cons_vec_0.push_back(Constraint{0, it->location, -1, it->level, constraint_type::VERTEX});
    
  }
  for (auto& it:cons_1){
    cons_vec_1.push_back(Constraint{1, it->location, -1, it->level, constraint_type::VERTEX});
  }
  if (reversed){
