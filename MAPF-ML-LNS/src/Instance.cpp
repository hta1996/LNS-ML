...
Instance::Instance(const string& map_fname, const string& agent_fname, int rand_agent,
	...
	map_fname(map_fname), agent_fname(agent_fname), num_of_agents(num_of_agents)
{
	...
	//cerr<<"tot_cell!"<<endl;
	tot_cell=0;
	//cerr<<num_of_rows<<" "<<num_of_cols<<endl;
	this->rand_agent=rand_agent;
...
{ 
	...
	//cerr<<num_of_cols<<" "<<num_of_rows<<endl;
...
void Instance::printGraphFeature(string filename)const
{
	...
	for (int i = 0; i < num_of_rows; i++)
	{
		for (int j = 0; j < num_of_cols; j++)
		{
			if (cell_index[i][j]==-1)continue;
			if (i-1>=0&&!my_map[linearizeCoordinate(i-1, j)])
				source.push_back(cell_index[i][j]),target.push_back(cell_index[i-1][j]);
			if (i+1<num_of_rows&&!my_map[linearizeCoordinate(i+1, j)])
				source.push_back(cell_index[i][j]),target.push_back(cell_index[i+1][j]);
			if (j-1>=0&&!my_map[linearizeCoordinate(i, j-1)])
				source.push_back(cell_index[i][j]),target.push_back(cell_index[i][j-1]);
			if (j+1<num_of_cols&&!my_map[linearizeCoordinate(i, j+1)])
				source.push_back(cell_index[i][j]),target.push_back(cell_index[i][j+1]);
		}
	}
	printf("[ %d", source[0]);
	for(int i=1;i<int(source.size());i++)printf(",%d",source[i]);
	printf("]\n");
	printf("[ %d", target[0]);
	for(int i=1;i<int(target.size());i++)printf(",%d",target[i]);
	printf("]\n");
	fclose(stdout);
}
	...
	if (line[0] == 'v') // Nathan's benchmark
	{
		...
		if(rand_agent)
		{
			...
			for (int i = 0; i < 999; i++)
			{
				...
				st.push_back(linearizeCoordinate(row, col));
				// read goal [row,col] for agent i
				beg++;
				col = atoi((*beg).c_str());
				beg++;
				row = atoi((*beg).c_str());
				gl.push_back(linearizeCoordinate(row, col));
			}
			//random_shuffle(idx.begin(),idx.end());
			int idx=0;
			for(int i=0;i<num_of_agents;i++)
			{
				start_locations[i]=st[idx],
				goal_locations[i]=gl[idx];
				idx=(idx+rand_agent)%int(st.size());
				//cerr<<start_locations[i]<<" "<<goal_locations[i]<<endl;
			}
			//exit(0);
		}else	
		{
